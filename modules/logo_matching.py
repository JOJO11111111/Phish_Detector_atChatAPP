...
#                     matched_target, matched_domain = matched_target, matched_domain  # keep them the same as they are
...
def check_domain_brand_inconsistency(logo_boxes,
    ...
    if len(logo_boxes) > 0:
        ...
        for i, coord in enumerate(logo_boxes):
            ...
            # If we got a match from the Siamese network
            if matched_target is not None:
                matched_coord = coord
                ...
                # Get all legitimate domains for this brand
                legit_domains = domain_map.get(matched_target, [])
                
                # Check if current domain matches any legitimate domain
                domain_match = any(
                    current_domain == legit_domain or 
                    extracted.domain == tldextract.extract(legit_domain).domain
