...
class SincConv(nn.Module):
    ...
    def forward(self,x):
        for i in range(len(self.mel)-1):
            ...
            hHigh = (2 * fmax / self.sample_rate) * np.sinc(2 * fmax * self.hsupp.detach().cpu().numpy() / self.sample_rate)
            hLow  = (2 * fmin / self.sample_rate) * np.sinc(2 * fmin * self.hsupp.detach().cpu().numpy() / self.sample_rate)
